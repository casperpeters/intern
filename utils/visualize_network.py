...
class Layer():
    ...
    def draw(self, layerType=0, weights=None, textoverlaphandler=None):
        ...
        if layerType == 0:
            pyplot.text(x_text, self.y, '$H_{t-1}$', fontsize = 12)
        elif layerType == -1:
            pyplot.text(x_text, self.y, '$V_{t}$', fontsize = 12)
        else:
            pyplot.text(x_text, self.y, '$H_{t}$'+str(layerType), fontsize = 12)
...
class NeuralNetwork():
    ...
    def draw(self, weights_list=None):
        ...
        pyplot.axis('scaled')
        pyplot.axis('off')
        pyplot.title( 'RTRBM architecture', fontsize=15 )
