...
class RTRBM(torch.nn.Module):
    def __init__(self, n_hidden, n_visible, T, dtype=torch.float):
        super().__init__()

        self.W = torch.nn.Parameter(torch.empty(n_hidden, n_visible, dtype=dtype))
        self.U = torch.nn.Parameter(torch.empty(n_hidden, n_hidden, dtype=dtype))
        self.b_h = torch.nn.Parameter(torch.empty(n_hidden, dtype=dtype))
        self.b_init = torch.nn.Parameter(torch.empty(n_hidden, dtype=dtype))
        self.b_v = torch.nn.Parameter(torch.empty(n_visible, dtype=dtype))
        self._init_params()
...
if __name__ == '__main__':
    ...
    #     n_populations=n_h, n_batches=20,
    #     time_steps_per_batch=1000,
    #     fr_mode='gaussian', delay=1, temporal_connections=temporal_connections, norm=0.4, spread_fr=[0.99, 1.01])
    #
    # gaus.plot_stats(T=100)
    # plt.show()
    # gaus_data = reshape(gaus.data)
    # train = reshape(gaus_data, T=100, n_batches=200)
    ...
    errors = learn(rtrbm, data=train, batch_size=10, n_epochs=1000, CDk=10, lr=1e-3, plot=True)
    # errors = learn(rtrbm, data=train, batch_size=10, n_epochs=1000, CDk=10, lr=1e-3,
    #                true_temporal_conn=gaus.temporal_connections, plot=True)
