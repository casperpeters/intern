        ...
        for epoch in tqdm(range(0, n_epochs), disable=self.disable):
            ...
            for batch in range(num_batches):
                ...
                dtheta_dw, dgamma_dtheta, dgamma_ddelta, dgamma_deta, dgamma_dw = self.get_cross_derivatives_dReLU(
                    v_data, I_data, self.gamma, self.theta, self.eta, self.delta)
                for i in range(0, batchsize):
                    ...
                    if torch.sum(torch.abs(dgamma)) > 1000:
                        a = 1
                    if torch.sum(torch.abs(dtheta)) > 1000:
                        a = 1
                    if torch.sum(torch.abs(ddelta)) > 1000:
                        a = 1
                    if torch.sum(torch.abs(deta)) > 1000:
                        a = 1
                    ...
                    dparam = [dW, db_V, dgamma, dtheta, ddelta, deta]
                ...
                # Recompute original parameters of HU dReLU potential for sampling
