...
class RTRBM(object):
    ...
    def visible_to_expected_hidden(self, v):
        ...
        for t in range(0, T):
            ...
            if t == 0:
                ...
                I = torch.matmul(v[:, t], self.VH.T) #+ torch.matmul(self.HH, H[:, t - 1])
                tp = self.theta_p
                tm = self.theta_m
                gp = self.gamma_p
                gm = self.gamma_m
    ...
    def visible_to_hidden(self, v, rt):
        ...
        for t in range(0, T):
            ...
            if t == 0:
                ...
                I = torch.matmul(v[:, t], self.VH.T) + torch.matmul(self.HH, rt[:, t - 1])
                tp = self.theta_p
                tm = self.theta_m
                gp = self.gamma_p
                gm = self.gamma_m
    ...
    def grad(self, vt, rt, ht_k, vt_k, barvt, barht, CDk):
        ...
        for t in range(self.T):
            if t==0:
                I_data = torch.matmul(vt[:, t], self.VH.T)
                I_model = torch.matmul(vt_k[:, t, -1], self.VH.T)
                param_t = self.grad_dReLU(I_data, I_model, t=0)
                for i in range(len(param_t)): param0[i] = param_t[i]

            elif t>0:
                I_data = torch.matmul(vt[:, t], self.VH.T) + torch.matmul(self.HH, rt[:, t - 1])
                I_model = torch.matmul(vt_k[:, t, -1], self.VH.T) + torch.matmul(self.HH, rt[:, t - 1])
        ...
        for t in range(self.T - 1, -1, -1): # begin, stop, step
            Dt[:, t] = torch.matmul(self.HH.T, (Dt[:, t + 1] * drt_drt_min_1[:, t] + (rt[:, t] - barht[:, t])))
        ...
        dVH_2 = torch.sum(rt.unsqueeze(1).repeat(1, self.N_V, 1) * vt.unsqueeze(0).repeat(self.N_H, 1, 1), 2) - \
               torch.sum(
        ...
        # Gradient for HH
        dHH = torch.sum((Dt[:, 2:self.T + 1] * (dr_dHH[:, 1:self.T]) + rt[:, 1:self.T] - barht[:, 1:self.T]).unsqueeze(
        ...
        return [dVH, dHH, db_V, dtheta_p0, dtheta_m0, dgamma_p0, dgamma_m0, dtheta_p, dtheta_m, dgamma_p, dgamma_m]

    def update_grad(self, Dparams, lr=1e-3, mom=0,wc=0, x=2, sp=None):

        dVH, dHH, db_V, dtheta_p0, dtheta_m0, dgamma_p0, dgamma_m0, dtheta_p, dtheta_m, dgamma_p, dgamma_m = self.dparams
        DVH, DHH, Db_V, Dtheta_p0, Dtheta_m0, Dgamma_p0, Dgamma_m0, Dtheta_p, Dtheta_m, Dgamma_p, Dgamma_m = Dparams
        ...
        Dparams = [DVH, DHH, Db_V, Dtheta_p0, Dtheta_m0, Dgamma_p0, Dgamma_m0, Dtheta_p, Dtheta_m, Dgamma_p, Dgamma_m]
