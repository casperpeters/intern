...
class RBM(object):
    ...
    def __init__(self, data, N_H=10, device=None):
        ...
        self.b_H = torch.zeros(self.N_H, 1, dtype=self.dtype, device=self.device)
        self.b_V = torch.zeros(self.N_V, 1, dtype=self.dtype, device=self.device)

        self.mu_H = torch.zeros(self.N_H, 1, dtype=self.dtype, device=self.device)
        self.mu_V = torch.zeros(self.N_V, 1, dtype=self.dtype, device=self.device)
    ...
    def visible_to_hidden(self, v, AF=torch.sigmoid):
        ...
        p = AF(torch.matmul(self.W, v) + self.b_H)
    ...
    def hidden_to_visible(self, h, AF=torch.sigmoid):
        ...
        p = AF(torch.matmul(self.W.T, h) + self.b_V)
    ...
    def grad(self, v_data, v_model, h_data, h_model):
        ...
        if self.dim == 2:
            db_V = torch.sum(v_data - v_model, 1)[:, None] - torch.matmul(dW.T, self.mu_H)
            db_H = torch.sum(h_data - h_model, 1)[:, None] - torch.matmul(dW, self.mu_V)
        elif self.dim == 1:
            db_V = v_data - v_model - torch.matmul(self.mu_H, dW)
            db_H = h_data - h_model - torch.matmul(self.mu_H, dW)
...
rbm.learn(batchsize=64, n_epochs=1000, lr=1e-2, lr_end=1e-4, start_decay=100, sp=1e-6, x=2)
...
print('sp: {}'.format(torch.sum(torch.abs(rbm.W) < 0.1) / (N_H * N_V)) )
