...
class RBM(torch.nn.Module):
    ...
    def free_energy(self, v):
        b_v_term = torch.dot(v, self.b_v)
        hidden_term = torch.sum(torch.log(1 + torch.exp(torch.dot(self.W, v) + self.b_h)), 1)
    ...
    def pseudo_loglikelihood_rbm(self, v):
        ...
        # f_total = torch.logaddexp(f_true, f_corrupted)
        # return torch.mean(v.shape[0] * (f_true - f_total))
...
class ShiftedRBM(RBM):
    ...
    def free_energy_given_r_lag(self, v, U, r_lag, b_init, t):
        if t == 0:
            hidden_term = torch.sum(torch.log(1 + torch.exp(torch.matmul(self.W, v) + b_init[:, None])), 0)
        else:
            hidden_term = torch.sum(torch.log(1 + torch.exp(torch.matmul(self.W, v) + torch.matmul(U, r_lag) + self.b_h[:, None])), 0)
...
class RTRBM(torch.nn.Module):
    ...
    def sample_h_given_v_over_time(self, v, U, b_init):
        ...
        for t, layer in enumerate(self.temporal_layers):
            if t == 0:
                r_lag, h[:, t, :] = [layer.sample_h_given_v_b_init(v[:, t, :], b_init)[1]]
            else:
                r_lag, h[:, t, :] = [layer.sample_h_given_v_r_lag(v[:, t, :], U, r_lag)[1]]
        return h
